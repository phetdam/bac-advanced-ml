...
\begin{enumerate}
    ...
    }{\texttt{numpy.hstack}} is very useful for creating matrices from smaller blocks. For example, let $ \mathbf{A} \in \mathbb{R}^{m \times n_1} $ be represented by \texttt{A}, a \texttt{numpy.ndarray} with shape \texttt{(m, n1)}, and let $ \mathbf{N} \in \mathbb{R}^{m \times n_2} $ be represented by \texttt{B}, a \texttt{numpy.ndarray} with shape \texttt{(m, n2)}. Then, \texttt{numpy.hstack((A, B))} returns a \texttt{numpy.ndarray} shape \texttt{(m, n1 + n2)} representing the block matrix $ \begin{bmatrix} \ \mathbf{A} & \mathbf{B} \ \end{bmatrix} $.
