...
\begin{frame}{Non-separable classes}
    \begin{itemize}
        ...
        \item
        Define $ \xi \succeq \mathbf{0} \in \mathbb{R}^N $, the ``slack''
        vector\footnote{
            From the Karush-Kuhn-Tucker conditions, $ \xi $ does actually
            define slack variables.
        }. One way to express this modification is to replace the linear
        constraints in (\aref{eq:max_margin_orig}) with
        \begin{equation*}
            \begin{split}
