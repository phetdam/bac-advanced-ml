...
\begin{frame}{Proximal gradient descent}
    ...
    % squeeze text a bit for aesthetic
    \medskip
\end{frame}
...
%    \begin{itemize}
%        \item
%        $ M_{\eta h} $, $ \operatorname{prox}_{\eta h} $ share many 
%        relationships. In particular, we note that \cite{prox_algos}
%        \begin{equation} \label{eq:prox_moreau_grad}
%            \operatorname{prox}_{\eta h}(\mathbf{x}) = \mathbf{x} -
%            \eta\nabla M_{\eta h}(\mathbf{x})
%        \end{equation}
%        Therefore, given an estimate $ \mathbf{x}_{t - 1} $ for a minimizer
%        $ \mathbf{x}^* $ of $ M_{\eta h} $, also a minimizer for $ h $,
%        $ \operatorname{prox}_{\eta h}(\mathbf{x}_{t - 1}) $ gives the
%        gradient descent update \cite{prox_algos}.
%
%        \item
