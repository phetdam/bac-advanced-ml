...
%
%        \item
%        (\aref{eq:prox_grad_update_accel_a}) shows that the gradient step in
%        $ \operatorname{prox}_{\eta_t h} $ is evaluated at an
%        \textbf{extrapolation} from $ \mathbf{x}_{t - 1} $ along
%        $ \mathbf{x}_{t - 1} - \mathbf{x}_{t - 2} $.
%
%        \item
%        Gradient steps are evaluated using
%        $ \{\tilde{\mathbf{x}}_t\}_{t \in \mathbb{N}} $, which from
%        (\aref{eq:prox_grad_update_accel_b}) zig-zags \textbf{more} than
%        $ \{\mathbf{x}_t\}_{t \in \mathbb{N}} $.
%
%        \item
%        (\aref{eq:prox_grad_update_accel_b}) shows that the extrapolation
%        increases with $ t $.
%    \end{itemize}
%\end{frame}
