...
% content in brackets is displayed in footer bar
...
% set graphics path since pdflatex is run using compile_tex.sh from the parent
% directory and gets confused on the graphics path.
%\graphicspath{{../}{bac_advanced_ml/}{../}}
            ...
            \While{not converged}{
                ...
                $
                    ...
                    \alpha\hat{\mathbf{m}}\oslash\big(
                        \sqrt{\hat{\mathbf{v}}} + \varepsilon\mathbf{1}
                    \big)
                $
            }
            % return statement
