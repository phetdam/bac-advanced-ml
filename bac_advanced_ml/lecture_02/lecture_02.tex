...
\begin{frame}{Norms and inner products}
    \begin{itemize}
        ...
        $ \mathbf{u}^\top\mathbf{v} = 0 $ and $ \Vert u\Vert =
        \Vert v\Vert = 1 $, where $ \Vert u'\Vert \triangleq
        \sqrt{\langle u', u'\rangle} $, $ \forall u' \in V $. $ u $ is a
        \textit{unit vector} if $ \Vert u\Vert = 1 $.
